...
@section('content')
        ...
        <form action="{{ route('products.store') }}" method="POST" class="space-y-4">
            ...
            <div> {{-- Selector para elegir el tipo de categoría que tienen los productos (enum)--}}
                ...
                <select id="category" name="category" class="w-full border p-2 rounded">
                    ...
                    <option value="Medicamentos" {{ request('category') == 'Medicamentos' ? 'selected' : '' }}>Medicamento
                    </option>
                ...
                <div id="vaccineinfo" class="mt-4 hidden space-y-4 border-t pt-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="validity_period" id="validity_period"
                            value="{{ old('validity_period') }}" class="w-full border p-2 rounded">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <button style="background-color: var(--color-button-secondary);" type="button"
                        onclick="openSupplierModal()" class="mt-2 bg-green-500 text-white px-3 rounded">Agregar
                        Proveedor</button>
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="total_cost_display" name="total_cost" class="w-full border p-2 rounded"
                        readonly>
                </div>
            ...
            <div class="bg-white rounded p-6 w-full max-w-md space-y-4">
                ...
                <form action="{{ route('suppliers.store.from.product') }}" method="POST">
                    ...
                    <input type="text" name="name" placeholder="Nombre de la Empresa" required
                        class="w-full border rounded p-2 my-2">
                    <input type="text" name="contact_name" placeholder="Nombre del Vendedor"
                        class="w-full border rounded p-2 my-2">
    ...
    <script>
        ...
        function openSupplierModal() {
            const productFormAction = "{{ route('products.store') }}";

            const inputs = document.querySelectorAll(
                `form[action="${productFormAction}"] input, 
                             form[action="${productFormAction}"] select, 
                             form[action="${productFormAction}"] textarea`
            );

            inputs.forEach(input => {
                if (input.name) {
                    localStorage.setItem('product_' + input.name, input.value);
                }
            });
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            inputs.forEach(input => {
                const saved = localStorage.getItem('product_' + input.name);
