...
<div class="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-md">
    <h1 class="text-2xl font-bold mb-6 text-gray-800">Editar Ficha Clínica</h1>
    ...
    <form action="{{ route('clinical_records.update', $clinicalRecord->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <select name="pet_id" id="pet_id" required
                ...
                @foreach ($pets as $pet)
                    <option value="{{ $pet->id }}"
                        {{ old('pet_id', $clinicalRecord->pet_id) == $pet->id ? 'selected' : '' }}>
        ...
        <div class="mb-4">
            ...
            <input type="datetime-local" id="date" name="date"
                value="{{ old('date', \Carbon\Carbon::parse($clinicalRecord->date)->format('Y-m-d\TH:i')) }}"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 w-full p-2.5"
                required>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="number" step="0.01" id="weight" name="weight"
                    value="{{ old('weight', $clinicalRecord->weight) }}"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 w-full p-2.5">
            </div>
            <div>
                ...
                <input type="number" step="0.1" id="temperature" name="temperature"
                    value="{{ old('temperature', $clinicalRecord->temperature) }}"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 w-full p-2.5">
            </div>
        </div>
        ...
        <div class="mb-4">
            ...
            <textarea id="symptoms" name="symptoms" rows="3"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 w-full p-2.5"
                required>{{ old('symptoms', $clinicalRecord->symptoms) }}</textarea>
        </div>
        ...
        <div class="mb-4">
            ...
            <textarea id="diagnosis" name="diagnosis" rows="3"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 w-full p-2.5">{{ old('diagnosis', $clinicalRecord->diagnosis) }}</textarea>
        </div>
        ...
        <div class="mb-4">
            ...
            <textarea id="treatment" name="treatment" rows="3"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 w-full p-2.5">{{ old('treatment', $clinicalRecord->treatment) }}</textarea>
        </div>
        ...
        <div class="mb-6">
            ...
            <textarea id="notes" name="notes" rows="2"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 w-full p-2.5">{{ old('notes', $clinicalRecord->notes) }}</textarea>
        </div>
