...
<div class="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-md">
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="{{ route('clinical_records.index') }}"
            class="text-sm text-indigo-600 hover:underline">&larr; Volver al listado</a>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div>
            ...
            <div class="text-sm text-gray-900"><strong>Fecha nacimiento:</strong> {{ $clinicalRecord->pet->date_of_birth ?? '-' }}</div>
        </div>

        <div>
            <h2 class="text-lg font-semibold text-gray-700 mb-2">Propietario</h2>
            <div class="text-sm text-gray-900"><strong>Nombre:</strong> {{ $clinicalRecord->pet->client->name }} {{ $clinicalRecord->pet->client->lastname }}</div>
            <div class="text-sm text-gray-900"><strong>Email:</strong> {{ $clinicalRecord->pet->client->email }}</div>
            <div class="text-sm text-gray-900"><strong>Teléfono:</strong> {{ $clinicalRecord->pet->client->phone ?? '-' }}</div>
            <div class="text-sm text-gray-900"><strong>Dirección:</strong> {{ $clinicalRecord->pet->client->address ?? '-' }}</div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div>
            ...
            <div class="text-sm text-gray-900"><strong>Fecha de atención:</strong> {{ $clinicalRecord->date->format('d/m/Y H:i') }}</div>
            <div class="text-sm text-gray-900"><strong>Veterinario:</strong> {{ $clinicalRecord->vet->name }} {{ $clinicalRecord->vet->lastname }}</div>
            <div class="text-sm text-gray-900"><strong>Peso:</strong> {{ $clinicalRecord->weight ? $clinicalRecord->weight . ' kg' : '-' }}</div>
            <div class="text-sm text-gray-900"><strong>Temperatura:</strong> {{ $clinicalRecord->temperature ? $clinicalRecord->temperature . ' °C' : '-' }}</div>
        </div>
    </div>
