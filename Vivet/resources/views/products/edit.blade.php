...
<div class="max-w-lg mx-auto">
    ...
    <form action="{{ route('products.update', $product) }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="number" name="price" id="price" step="0.01" value="{{ old('price', $product->price) }}" class="w-full border p-2 rounded" required>
        </div>

        <div>
            <label for="stock" class="block font-medium">Stock</label>
            <input type="number" name="stock" id="stock" value="{{ old('stock', $product->stock) }}" class="w-full border p-2 rounded" required>
        </div>
