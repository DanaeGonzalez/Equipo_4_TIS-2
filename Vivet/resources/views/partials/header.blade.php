  ...
  <div class="mx-auto max-w-screen-xl px-4 md:px-8">
    <header class="flex items-center justify-between py-3 md:py-4">
      ...
      <nav class="hidden gap-12 lg:flex">
        ...
        @auth
          @if(in_array(auth()->user()->role->name, ['Administrador', 'Veterinario']))
            <a href="{{ route('schedules.index') }}"
              class="text-lg font-semibold text-gray transition duration-100 hover:underline">
               Agenda
            </a>
          @endif
        @endauth
      ...
      @csrf
      <button type="submit" class="inline-block rounded-lg px-4 py-3 text-center text-sm font-semibold text-gray-500 outline-none transition duration-100 hover:opacity-80 focus-visible:ring active:opacity-90 md:text-base">Cerrar Sesión</>
      </form> -->

      <!-- Dropdown button -->
      <div class="relative">
      <button id="userDropdownButton" data-dropdown-toggle="userDropdown"
        class="flex items-center gap-2 rounded-lg px-4 py-3 text-sm font-semibold text-gray-500 outline-none transition duration-100 hover:opacity-80 focus-visible:ring active:opacity-90 md:text-base">
        <!-- User Avatar -->
