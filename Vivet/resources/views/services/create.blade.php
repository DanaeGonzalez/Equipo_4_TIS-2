...
<div class="max-w-lg mx-auto">
    ...
    <form action="{{ route('services.store') }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="number" name="estimated_duration" id="estimated_duration" value="{{ old('estimated_duration') }}" class="w-full border p-2 rounded" required>
        </div>
        ...
        <div>
            <label for="icon" class="block font-medium">Icono (opcional)</label>
            <input type="text" name="icon" id="icon" value="{{ old('icon') }}" class="w-full border p-2 rounded">
        </div>
